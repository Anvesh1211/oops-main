...
AUTH_FRONTEND_DIR = os.path.join(BASE_DIR, 'FRONTEND')
...
def login_page():
    return send_from_directory(AUTH_FRONTEND_DIR, 'login.html')

@app.route('/register.html')
def register_page():
    return send_from_directory(AUTH_FRONTEND_DIR, 'register.html')

@app.route('/dashboard_student.html')
def student_dashboard_page():
    return send_from_directory(AUTH_FRONTEND_DIR, 'dashboard_student.html')

@app.route('/dashboard_faculty.html')
def faculty_dashboard_page():
    return send_from_directory(AUTH_FRONTEND_DIR, 'dashboard_faculty.html')

@app.route('/auth.js')
def auth_js():
    return send_from_directory(AUTH_FRONTEND_DIR, 'auth.js')


@app.route('/<path:path>')
def serve_static(path):
    # Try FRONTEND dir first, fall back to original frontend
    auth_file = os.path.join(AUTH_FRONTEND_DIR, path)
    if os.path.exists(auth_file):
        return send_from_directory(AUTH_FRONTEND_DIR, path)
...
if __name__ == '__main__':
    ...
    print("  Flask Backend Server + Auth System")
    print("="*60)
    ...
    print(f"  Auth Pages: login.html, register.html")
    print(f"  Dashboards: dashboard_student.html, dashboard_faculty.html")
    try:
        s = get_booking_stats()
