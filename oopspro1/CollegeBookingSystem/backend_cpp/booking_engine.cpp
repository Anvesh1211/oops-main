/*
 * ============================================================
 *  booking_engine.cpp — C++ OOP Backend Engine with JSON I/O
 * ============================================================
 *  OOP Concepts Demonstrated:
 *    1. Abstraction      - Pure virtual in User base class
 *    2. Inheritance       - Student, Faculty from User
 *    3. Encapsulation     - Private data with getters/setters
 *    4. Polymorphism      - Virtual dispatch
 *    5. Composition       - BookingEngine HAS-A RoomGenerator
 *    6. Dynamic Objects   - Runtime vector management
 *    7. File Handling     - SQLite database operations
 *
 *  This engine reads JSON commands from stdin and writes
 *  JSON responses to stdout for Flask integration.
 *
 *  Build:
...
// ── BookingEngine class (Composition + Encapsulation) ──
...
private:
    ...
    // Check if room+date+slot is booked
    ...
    // Find closest available room using KNN logic
    ...
    // Find next available slot for same room
    ...
    // Load bookings from a simple text-based file (for simplicity/compatibility)
    void loadFromFile() {
        bookings.clear();
...
public:
    ...
    // ── Check availability and return JSON result ──
    std::string checkAndBook(const std::string& user, int room,
        ...
        // Room is booked — find alternatives
    ...
    // ── Get room status grid for a date ──
    std::string getRoomStatus(const std::string& date) {
        // Build a map of room -> slot -> status
        ...
        // Output first 100 rooms that have bookings, or a sample
        const auto& allRooms = roomGen.getRooms();
        bool first = true;

        // Show rooms that have at least one booking on this date
        for (auto& pair : grid) {
            if (!first) oss << ",";
